...
const MatchMini = ({
  ...
  const summonerSpell = (spellID) => {
    const spellURL =
      "http://ddragon.leagueoflegends.com/cdn/13.10.1/img/spell/";
  ...
  return (
    ...
    <div
      className={
        durationMin > 3
          ? matchSpecs.win
            ? "profile_body_match victory_match"
            : "profile_body_match defeat_match"
    ...
    >
      <div className="profile_body_match_specs queue">
        <p
          className={
            durationMin > 3
              ? matchSpecs.win
                ? "queue_type type_victory"
                : "queue_type type_defeat"
              : "queue_type type_remake"
          }
        >
          {queueType(gameMode)}
        </p>
        <p className="queue_result">
          {durationMin > 3 ? (matchSpecs.win ? "Victory" : "Defeat") : "Remake"}
        </p>
        <p className="queue_match_duration">
          {durationMin}m {durationSeg}s
        </p>
      </div>
      <div className="profile_body_match_specs champion">
        <p
          className={
            durationMin > 3
              ? matchSpecs.win
                ? "queue_type_mini type_victory"
                : "queue_type_mini type_defeat"
              : "queue_type_mini type_remake"
          }
        >
          {queueType(gameMode)}
        </p>
        <div className="champion_champion">
          ...
          <div className="profile_body_match_specs scores">
            <div className="scores_kda">
              {matchSpecs.kills}/{matchSpecs.deaths}/{matchSpecs.assists}
            </div>
            <div className="scores_kda-points">{matchSpecs.kda} KDA</div>
            <div
              className="scores_killing-spree"
              style={
                killingSpree(matchSpecs.multikills) ? {} : { display: "none" }
              }
            >
              {killingSpree(matchSpecs.multikills)}
            </div>
          </div>
        </div>
              ...
              return (
                <div className="build_item" key={index}>
                  {item > 0 ? (
                    <img
                      src={
                        "http://ddragon.leagueoflegends.com/cdn/13.10.1/img/item/" +
                        item +
                        ".png"
                      }
                      alt="xd"
      ...
      <div className="profile_body_match_specs general_stats">
        <p className="score_killP">
          P/Kill{" "}
          {matchSpecs.killParticipation
            ? (matchSpecs.killParticipation.toFixed(3) * 100).toFixed(1)
            : matchSpecs.killParticipation}
          %
        </p>
        <p className="score_visionScore">
          Vision Score {matchSpecs.visionScore}
        </p>
              ...
              return (
                <div className="team_member" key={user.summonerName}>
                  <div
                    ...
                    className={
                      user.summonerName == matchSpecs.summonerName
                        ? "team_icon_member selected_summoner"
                        : "team_icon_member"
                    }
                  >
