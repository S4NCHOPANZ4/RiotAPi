...
const Players = () => {
  ...
  useEffect(() => {
    ...
    getLeaderboars(server.server).then((res) => {
      const sortedPlayers = res.entries.sort(
        (p1, p2) => p2.leaguePoints - p1.leaguePoints
      );
      ...
      Promise.all(
        topPlayersPre.map((player, index) => {
          return getPLayerByName(player.summonerName, server.server).then(
            (res) => {
              if (res === "err") {
  ...
  return (
    <>
      ...
      ) : leaderdoard.length <= 0 ? (
        <div
          className="leadderboards_nosummoners"

        >
          <h1>No Summoners on challenger yet :c</h1>
        </div>
      ) : (
              ...
              <div className="top_0">
                ...
                <div className="imgContainer_top0">
                  <img
                    src={
                      ...
                      topPlayers[0].summoner.profileIconId +
                      ".png"
                    }
                    alt=""
                  />
                ...
                <div className="top_0_data">
                  <p
                    onClick={() => {
                      gotoProfile(
                        leaderdoard[0].summonerName,
                        server.server,
                        server.continent
                      );
                    }}
                    className="top_0_summonerName"
                  >
                    {leaderdoard[0].summonerName}{" "}
                    <span className="lps">
                      {topPlayers[0].rank.leaguePoints} LP
                    </span>
                  </p>
                  <div className="top_0_winrate">
                    <div className="top_0_winrate_tier">
                      <img
                        src="https://raw.communitydragon.org/13.9/plugins/rcp-fe-lol-shared-components/global/default/images/challenger.png"
                        alt=""
                      />
                    </div>
                    <div className="top_0_winrate_side">
                      <p className="top_0_winrate_stats">
                        <span className="top_0_winrate_per">
                          {getWinratePer(
                            topPlayers[0].rank.wins,
                            topPlayers[0].rank.losses
                          )}
                          %
                        </span>
                        /{topPlayers[0].rank.wins + topPlayers[0].rank.losses}{" "}
                        games
                      </p>
                ...
                {topPlayers.map((plyr, index) => {
                  if (index === 0) {
                    ...
                    return (
                      <div
                        key={index}
                        className={`${"top_" + index} sub_top_card`}
                      >
                        <div className="sub_card_topSide">
                          <p className={`sub_card_rank ${"rank" + index + 1}`}>
                            {index + 1}
                          </p>
                          ...
                          <div className="sub_card_lower">
                            <p
                              ...
                              className="sub_card_summonerName"
                            >
                              {plyr.summoner.name}
                            </p>
                            <p className="subcard_specs">
                              <span className="subcard_winratio">
                                {getWinratePer(
                                  plyr.rank.wins,
                                  plyr.rank.losses
                                )}
                              ...
                              <span className="subcard_games">
                                {plyr.rank.wins + plyr.rank.losses} games
                              </span>
                            </p>
                            <div className="lower_card_lowerbar">
                              <div
                                className="lower_card_upperbar"
                                style={{
                                  width: `${getWinratePer(
                                    plyr.rank.wins,
                                    plyr.rank.losses
                        ...
                        <div className="sub_card_midSide">
                          ...
                          <p className="midSide_elo">
                            ...
                            <span className="bar">/</span>
                            <span className="elo_points">
                              {plyr.rank.leaguePoints} LP
                            </span>
                          </p>
                        </div>
                      </div>
          ...
          <div className="leaderboards_table">
            {leaderdoard.slice(minRank, maxRank).map((elemento, index) => (
              <div
                ...
                className={
                  index % 2 === 0
                    ? "leaderboards_item pair_item"
                    : "leaderboards_item even_item"
                }
              >
                ...
                <div className="item_summoner_winratio">
                  <p>
                    <span
                      className={
                        getWinratePer(elemento.wins, elemento.losses) >= 60
                          ? "winratrio_per per_positive"
                          : "winratrio_per"
                      }
                    >
                      {getWinratePer(elemento.wins, elemento.losses)}%
                    </span>{" "}
                    <span className="winratio_wins winratio_stat">
                      {elemento.wins}W
                    </span>{" "}
                    <span className="winratio_losses winratio_stat">
                      {elemento.losses}L
                    </span>
                  </p>
                  <div className="winratio_lower_bar">
                    <div
                      className="winratio_upper_bar"
                      style={{
                        width: `${getWinratePer(
                          elemento.wins,
                          elemento.losses
